...
export default function RegisterFormCustomer({ onBack }: Props) {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        await registerUser({ ...values, role: 'CUSTOMER' });
        setSuccess(true);
        setTimeout(() => {
          router.push('/login');
        }, 2000);
  ...
  return (
    <div className="bg-white p-8 rounded-xl shadow max-w-md w-full">
      ...
      <h2 className="text-2xl font-bold mb-4 text-amber-700">Register as Customer</h2>
